...
export function DisputesPageClient({
  ...
  disputed: DisputeJobRow[]
  openishCount: number
}) {
  const t = useT()
  const p = t.disputesPage
    ...
    <div className="min-h-[100dvh]">
      ...
      <section className="page-container py-8 md:py-14">
        ...
        <div className="mb-8 max-w-[60ch] animate-fade-up">
          ...
          <h1 className="text-2xl font-bold tracking-tight sm:text-3xl md:text-[2.6rem] md:leading-[1.05]">{p.title}</h1>
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{p.body}</p>
        </div>

        <div className="mb-8 grid gap-4 sm:grid-cols-3">
          {[
            { icon: Scale, label: p.disputed, value: disputed.length, tone: "text-red-400" },
            { icon: Shield, label: p.activeJobs, value: openishCount, tone: "text-primary" },
            { icon: Gavel, label: p.council, value: p.live, tone: "text-yellow-500" },
          ].map((s) => {
            const Icon = s.icon
            return (
              ...
              {disputed.length === 0 ? (
                <div className="flex flex-col items-center gap-2 py-12 text-center text-sm text-muted-foreground">
                  ...
                  <Button asChild variant="outline" className="mt-2 rounded-full">
                    <Link href="/jobs">{p.browseTasks}</Link>
                  </Button>
                </div>
              ) : (
            ...
            <Card className="surface-card border border-border/60">
              <CardContent className="space-y-3 p-5 text-sm">
                ...
                <Button asChild className="w-full rounded-full">
                  <Link href="/join/evaluator">{p.becomeEval}</Link>
                </Button>
              </CardContent>
            </Card>
            <Card className="surface-card border border-border/60">
