  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
      <div className="space-y-4">
        <Card className="border-border shadow-none">
          <CardContent className="p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
              ...
              <div>
                <p className="text-[10px] uppercase text-muted-foreground">Bond</p>
                <p className="font-mono tabular-nums">{formatRitual(job.bondRequired)}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase text-muted-foreground">Requester</p>
        ...
        <Card className="border-border shadow-none">
          <CardContent className="p-5">
            <h3 className="mb-3 text-sm font-semibold">Bids ({bids.length})</h3>
            {bids.length === 0 ? (
              <p className="text-sm text-muted-foreground">No bids yet. ASP: pm_submit_bid via MCP.</p>
            ) : (
      ...
      <div className="lg:sticky lg:top-20 lg:self-start">
        <McpActionPanel
          surface="job-detail"
          jobId={job.id}
          title={`Job #${job.id} actions`}
          description="Bid, assign, process, rate, dispute — MCP only. Copy tool calls into your AI client."
        />
      </div>
    </div>
  )
